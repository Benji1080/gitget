## ⚡ دانلود فایل از طریق سرور گیت هاب و میرور کردن در رپو برای رهایی از محدودیت ها

اگر میخواید یک فایل از اینترنت دانلود کنید چه گیت هاب یا هر سایت دیگه، ولی بهش دسترسی ندارید، کافیه لینکش رو به این اکشن بدید تا با سرور گیت هاب دانلود کنه داخل رپوی مورد نظرتون بعد از اونجا میتونید دانلودش کنید.

1. در ریپوی خودتون یه فایل در این آدرس ایجاد کنید: `github/workflows/gitget.yml`.
2. فایل رو ویرایش کنید و کد زیر رو بذارید داخلش و فایل رو کامیت و ثبت کنید.
3. بعد برید قسمت اکشن ها و این اکشن جدید رو اجرا کنید و لینک مستقیم فایلی که قراره دانلود کنید رو وارد کنید، چه از گیتهاب یا هرجا در اینترنت!
    ...
    steps:
      ...
      - name: Download and process file
        ...
        run: |
          ...
          # Clean previous leftovers for this exact filename
          ...
          # Split if > 95 MiB (safe under GitHub's 100 MB limit)
          SAFE_MAX=$((95 * 1024 * 1024))
          if [ "$FILESIZE" -gt "$SAFE_MAX" ]; then
            log "[INFO] File exceeds 95 MiB, splitting into 95 MiB multi‑part ZIP"
            cd "$DOWNLOAD_DIR"
            zip -0 -s 95m -r "${FILENAME}.split.zip" "$FILENAME" 2>&1 | tee -a "../$LOG_FILE"
            ...
            PARTS_INFO=""
            # Only iterate over .split.zip and then numbered parts (.z01, .z02, ...)
            for part in "$DOWNLOAD_DIR/$FILENAME.split.zip" "$DOWNLOAD_DIR/$FILENAME".split.z[0-9]*; do
              [ -f "$part" ] || continue
            ...
            log "[INFO] File within safe limit, keeping original"
